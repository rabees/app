    ...
    public function handle(Request $request, Closure $next)
    {
        if (!Str::contains($request->path(), ['install'])) {
            ...
            if ($license_notifications_array['notification_case'] != "notification_license_ok") {
                abort(403);
            } else if (isset($license_notifications_array['notification_data']) && json_decode($license_notifications_array['notification_data']) != "Extended License" && getSetting('PAYMENT_MODE') == 'enabled') {
